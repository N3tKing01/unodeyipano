...
async function tiktokCommand(sock, chatId, message) {
    try {
        ...
        if (!url) {
            return await sock.sendMessage(chatId, { 
                text: "❌ Sicela unikeze isixhumanisi se-TikTok ngemuva komyalo."
            }, { quoted: fakeQuoted });
        }
        ...
        if (!isValidUrl) {
            return await sock.sendMessage(chatId, { 
                text: "❌ Leso akusona isixhumanisi se-TikTok esivumelekile. Sicela unikeze isixhumanisi sevidiyo se-TikTok esivumelekile."
            }, { quoted: fakeQuoted });
        }
        ...
        try {
            ...
            if (data && data.status && data.result && data.result.result) {
                ...
                caption += `> Powered by MidKing API`;
            ...
            try {
                ...
                const fallbackResponse = await axios.get(
                    `https://www.cod3uchiha.com/downloaders/tiktokdl?url=${encodeURIComponent(url)}`,
                    { timeout: 30000 }
                );
                ...
                await sock.sendMessage(chatId, {
                    text: "❌ Yehlulekile ukulanda ividiyo ye-TikTok. Sicela uzame futhi kamuva"
                }, { quoted: fakeQuoted });
        ...
        await sock.sendMessage(chatId, {
            text: "❌ Kuvele iphutha elingalindelekile. Sicela uzame futhi."
        }, { quoted: fakeQuoted });
    }
}
