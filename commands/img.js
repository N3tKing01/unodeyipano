...
async function searchImagesFromAPI(query, apiType) {
    try {
        ...
        if (apiType === 'cod3uchiha') {
            const url = `https://www.cod3uchiha.com/downloaders/img?text=${encodeURIComponent(query)}`;
...
async function imgCommand(sock, chatId, senderId, message, userMessage) {
    try {
        ...
        let images = await searchImagesFromAPI(query, 'Cod3uchiha');
        let usedAPI = 'TakudzwaMlambotech';
        ...
        if (images.length === 0) {
            return await sock.sendMessage(chatId, {
                text: '❌ Azikho izithombe ezitholakele zombuzo wakho. Zama amagama angukhiye ahlukene.'
            }, { quoted: fake });
        }
        ...
        if (sentCount > 0) {
            ...
            await sock.sendMessage(chatId, {
                text: '❌ Yehlulekile ukuthumela izithombe. Sicela uzame futhi.'
            }, { quoted: fake });
        }
        ...
        const fake = createFakeContact(message);
        await sock.sendMessage(chatId, {
            text: '❌ Iphutha ekufuneni izithombe. Sicela uzame futhi.'
        }, { quoted: fake });
        await sock.sendMessage(chatId, {
            react: { text: '❌', key: message.key }
        });
