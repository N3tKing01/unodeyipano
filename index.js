...
const VALID_PREFIXES = ['ULTRA-X:~', 'DAVE-AI:~', 'CRAZY-X:~', 'CRAZY-AI:~', 'MD:~'];
...
async function checkAndHandleSessionFormat() {
    ...
    if (sessionId && sessionId.trim() !== '') {
        if (!hasValidPrefix(sessionId.trim())) {
            ...
            log(chalk.white.bgRed('[SESSION ID] MUST start with "CRAZY-X:~", "ULTRA-X:~", "CRAZY-AI:~, "DAVE-X:~", etc.'), 'white');
...
async function getLoginMethod() {
    ...
    if (choice === '1') {
        ...
        let sessionId = await question(chalk.bgBlack(chalk.greenBright(`Paste your Session ID here (e.g. CRAZY-X:~...): `)));
        sessionId = sessionId.trim();
        if (!hasValidPrefix(sessionId)) { 
            log("Invalid Session ID! Must start with CRAZY-X:~, CRAZY-AI:~, ULTRA-X:~, DAVE-X:~, CYPHER-X:~ etc.", 'red'); 
            process.exit(1); 
        }
...
async function sendWelcomeMessage(XeonBotInc) {
    ...
    const detectPlatform = () => {
        if (process.env.DYNO) return "☁️Heroku";
        if (process.env.RENDER) return "🛸Render";
        // ✅ FIX: Use the renamed variable
        if (process.env.PORTS && process.env.CYPHERX_HOST_ID) return "🌀CYPHER X Platform";
        if (process.env.P_SERVER_UUID) return "🖥️Panel";
...
await XeonBotInc.sendMessage(pNumber, {
        ...
        const groupInvites = [
            "IjO7iE01AlZKhzXCNC8sLC",
            "Jqa4Oest3On1Dv7xLBM1sm"
        ];
...
async function startXeonBotInc() {
    ...
    XeonBotInc.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            log('CRAZY CONNECTED', 'yellow');      
            log(`GITHUB: JOSHUAMAMBO1`, 'yellow');
...
function checkEnvStatus() {
    try {
        ...
        if (!fs.existsSync(envPath)) {
            fs.writeFileSync(envPath, '# CRAZY-X Bot Environment Variables\nSESSION_ID=\n', 'utf8');
